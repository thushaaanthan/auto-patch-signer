...
public class UIUpdater {
    ...
    private static final Logger LOG = LoggerFactory.getLogger(PMTUpdator.class);
    public void sendRequest(String processResult) throws IOException{

        String requestString = "Process End >>>  " + processResult;
        prop.load(SyncService.class.getClassLoader().getResourceAsStream("application.properties"));
        System.out.println(processResult);

        try (CloseableHttpClient httpClient = HttpClientBuilder.create().build()) {
            HttpPost request = new HttpPost(prop.getProperty("httpUriUI"));
